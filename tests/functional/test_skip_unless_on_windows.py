...
def test_error_on_args_or_kwargs(pytester):
    pytester.makepyfile(
        ...
        @pytest.mark.skip_unless_on_windows("arg")
        def test_one():
            assert True

        @pytest.mark.skip_unless_on_windows(kwarg="arg")
        def test_two():
            assert True
        """
    )
    ...
    res.stdout.fnmatch_lines(
        [
            "*UsageError: The skip_unless_on_windows marker does not accept any arguments",
            "*UsageError: The skip_unless_on_windows marker only accepts 'reason' as a keyword argument.",
        ]
    )
