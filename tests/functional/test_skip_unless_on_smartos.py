...
def test_error_on_args_or_kwargs(pytester):
    pytester.makepyfile(
        ...
        @pytest.mark.skip_unless_on_smartos("arg")
        def test_one():
            assert True

        @pytest.mark.skip_unless_on_smartos(kwarg="arg")
        def test_two():
            assert True
        """
    )
    ...
    res.stdout.fnmatch_lines(
        [
            "*UsageError: The skip_unless_on_smartos marker does not accept any arguments",
            "*UsageError: The skip_unless_on_smartos marker only accepts 'reason' as a keyword argument.",
        ]
    )
