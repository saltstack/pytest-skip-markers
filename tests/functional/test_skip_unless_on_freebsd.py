...
def test_skip_reason(pytester):
    pytester.makepyfile(
        ...
        @pytest.mark.skip_unless_on_freebsd(reason='Because!')
        def test_one():
            assert True
        """
    )
...
def test_error_on_args_or_kwargs(pytester):
    pytester.makepyfile(
        ...
        @pytest.mark.skip_unless_on_freebsd("arg")
        def test_one():
            assert True

        @pytest.mark.skip_unless_on_freebsd(kwarg="arg")
        def test_two():
            assert True
        """
    )
    ...
    res.stdout.fnmatch_lines(
        [
            "*UsageError: The skip_unless_on_freebsd marker does not accept any arguments",
            "*UsageError: The skip_unless_on_freebsd marker only accepts 'reason' as a keyword argument.",
        ]
    )
